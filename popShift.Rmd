...
Here we define author retention as the overlap in authors publishing at the journal in a given year relative to the previous threes years (the choice of three is relatively arbitrary). We calculate an 'author retention index' as the Dice coefficient between each target year and the previous three years. If there is a general population shift caused by the policy intervention, we might expect to see a marked reduction in the author retention index shortly after the policy was introduced.
